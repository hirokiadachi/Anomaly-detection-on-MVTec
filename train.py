...
def training_ae(epoch, model, dataloader, optimizer):
    model.train()
    for idx, (inputs, _) in enumerate(dataloader):
        inputs = inputs.cuda()
        output = model(inputs)
        loss = torch.sum((inputs - output)**2)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        
        if idx % 100 == 0:
            print('%d epoch [%d/%d] | loss: %.4f |' % (epoch, idx, len(dataloader), loss.item()))

def training_vae(epoch, model, dataloader, optimizer):
    model.train()
    for idx, (inputs, _) in enumerate(dataloader):
        inputs = inputs.cuda()
        output = model(inputs)
...
if __name__ == '__main__':
    ...
    print('Start training with %s (category: %s)' % (args.model_type.upper, args.category))
    np.random.seed(args.seed_numpy)
    torch.manual_seed(args.seed_torch)
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
    save_dir = os.path.join(args.checkpoint, args.category, args.model_type)
    ...
    if args.model_type == 'vae':
        ...
        training = training_vae
    elif args.model_type == 'ae':
        model = AE(z_dim=args.z_dim, input_c=channel).cuda()
